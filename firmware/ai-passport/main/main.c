...
static const demo_entry_t DEMOS[] = {
    { "Talk",    demo_talk_enter,    demo_talk_exit,    demo_talk_key    },
...
static void on_key(bsp_btn_t btn, bsp_btn_ev_t ev, void *user) {
    (void)user;
    /* HID must not wait on LVGL lock — otherwise keys vanish. */
    if (s_active == 0 && ev == BSP_BTN_PRESS) {
        if (btn == BSP_BTN_UP)   ble_hid_tap_up();
        if (btn == BSP_BTN_DOWN) ble_hid_tap_down();
    }
...
void app_main(void) {
    ...
    if (ble_hid_kbd_start() != ESP_OK) {
        ESP_LOGE(TAG, "BLE HID 启动失败");
    }
    ...
    s_ok[0] = true;     /* Talk / BLE key — 不依赖麦克风 */
    s_ok[1] = true;     /* Display */
    s_ok[2] = btn_ok;
    s_ok[3] = aud_ok;
    s_ok[4] = bat_ok;
    ...
    ESP_LOGI(TAG, "就绪:Talk=%d Display=1 Button=%d Audio=%d Battery=%d",
